...
@onready var animation_fp := $animation_fp as AnimationPlayer
...
func has_collided_with(collision: KinematicCollision2D, collider: CharacterBody2D):
	if !is_triggered:
		is_triggered = true
		animation_fp.play("shake")
		velocity = Vector2.ZERO
		

func _on_animation_fp_animation_finished(anim_name: StringName) -> void:
...
func _on_respawn_timer_timeout() -> void:
	...
	if is_triggered:
		...
		spawn_tween.tween_property($sprite_fp, "scale", Vector2(1, 1), 0.2).from(Vector2(0,0))
	is_triggered = false
